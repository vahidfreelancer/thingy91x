 ...
 */
enum app_wifi_state {
    APP_WIFI_STATE_DISCONNECTED = 0,    /**< Wi-Fi interface idle / disconnected */
    APP_WIFI_STATE_SCANNING,            /**< Active or passive channel scan in progress */
    APP_WIFI_STATE_CONNECTING,          /**< Connecting / authenticating to Wi-Fi AP */
    APP_WIFI_STATE_CONNECTED            /**< Connected with IP address assigned */
};
...
struct wifi_scan_data {
    ...
    struct wifi_ap_info results[8];     /**< Access points array */
 ...
 */
struct app_wifi_status {
    enum app_wifi_state state;  /**< Current Wi-Fi state */
 ...
 * 
 * @param status Pointer to app_wifi_status structure to populate.
 * @return 0 on success, negative POSIX error code on failure.
 */
